...
sys.path.append(os.path.join(BASE_DIR,'../../show_demo'))
import ImgStream 
...
def demo(configs):
    ...
    with torch.no_grad():
        for count, resized_imgs in video_loader:
            asyncio.sleep(0.001)
            if Thread_Contral.get_thread_status() == "RUN":
                # take the middle one
                ...
                if len(queue_frames) == middle_idx + 1:
                    ...
                    if configs.show_image:
                        ...
                        # cv2.waitKey(10)
                        # ploted_img = cv2.imread(os.path.join(BASE_DIR,'../../videos/test.jpg'))
                        pass 
                    if configs.save_demo_output:
                        # cv2.imwrite(os.path.join(configs.frame_dir, '{:06d}.jpg'.format(frame_idx)), ploted_img)
                        # cv2.imwrite(os.path.join(configs.frame_dir, '1.jpg'), ploted_img)
                        # time.sleep(5*0.001)  
                        # ploted_img = cv2.imread(os.path.join(configs.frame_dir, '1.jpg'))
                        ImgStream.send_img_to_stream(ploted_img) 
                        pass 
            ...
            elif Thread_Contral.get_thread_status() == "STOP":
                while Thread_Contral.get_thread_status() == "STOP":
                    asyncio.sleep(0.001)
                    pass
                pass
            elif Thread_Contral.get_thread_status() == "BREAK":
                break
    print('demo stop')
